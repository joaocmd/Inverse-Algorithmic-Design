...
thinwall([p19, p28])
thinwall([p20, p21])
# thinwall([p20, p29]) # moved below
thinwall([p110, p29])
thinwall([p22, p23])
thinwall([p23, p24])
thinwall([p23, p34])
thinwall([p24, p35])
# thinwall([p25, p30]) # removed
thinwall([p26, p31])
thinwall([p27, p37])
thinwall([p28, p29])
thinwall([p28, p33])
...
thinwall([p60, p66])
# thinwall([p64, p65]) removed because coinciding
thinwall([p65, p66])
...
# railing([p39, p40], 0.12) replaced with wall
thinwall([p39, p40])
# railing([p58, p62], 0.12) discarded yet correct
...
curtain_wall([p36, p50])
curtain_wall([p49, p50])
curtain_wall([p53, p92])
